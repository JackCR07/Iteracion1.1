...
CREATE TABLE `cii_contrato` (
  `ID` int(11) NOT NULL AUTO_INCREMENT,
  `NombreNiño` varchar(40) NOT NULL,
...
CREATE TABLE `cii_funcionario` (
  `ID` int(11) NOT NULL AUTO_INCREMENT,
  `Nombre` varchar(30) NOT NULL,
  `Contraseña` varbinary(128) DEFAULT NULL,
...
INSERT INTO `cii_usuario` VALUES (1,'Ricardo','ØîÆ-|p½KCy¸À+Á');
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `NuevoContrato`(in niño varchar(40),in cost float,in funcionarioid varchar(30),in user varchar(15),out res bool, out idc int)
BEGIN
declare idf int;
declare idu int;
set res=false;
...
set res=true;
Insert into cii_contrato(nombreniño,costo,cii_funcionario_id,fk_cii_usuario_id)
values(niño,cost,idf,idu);
...
Declare idtf int; 
set res=FALSE;
set temp=(select id from CII_Funcionario where Nombre=nom and Contraseña=AES_ENCRYPT(pass,'supreme'));
